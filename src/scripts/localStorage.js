...
let allDone = document.querySelector('.page')
console.log(allDone)
let isReadmePresent = (AllPages.some((page) => page.name === 'Readme: tasks') || AllPages.some((page) => page.name === 'Readme: pages') || AllPages.some((page) => page.name === 'Readme: misc'))
if (!isReadmePresent) {
    const part1 = [
    ...
    const part5 = [
        "This site can be installed as an app on mobile, via the 'Add to home screen/Install app' option in your browser",
        "Doing so adds a 'doBetter' icon to your home screen for quick access",
        "Further more, it gives the app a native look eliminating the browser bar"
    ];
    const part6 = [
